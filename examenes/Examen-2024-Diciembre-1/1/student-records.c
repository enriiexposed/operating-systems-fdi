...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt(argc, argv, "hi:po:ba")) != -1) {
        switch (opt) {
            ...
            case 'o':
                ...
                break;
            case 'b':
                options.action = COMANDO_B;
                break;
            case 't':
                options.action = COMANDO_T;
                break;
            case 'a':
                options.action = COMANDO_A;
                break;
            default:
                exit(EXIT_FAILURE);
        }
    ...
    switch (options.action) {
        ...
        case COMANDO_B:
            ...
            int pid = fork();
            if (pid == 0) {
                char* arr[] = {"./student-records", "-i", "students-db.txt", "-o", options.output_file, NULL};
                execv("./student-records", arr);
            } else {
                waitpid(pid, &status, 0);
            } 
            char* arrdiff[] = {"diff", options.input_file, options.output_file, NULL};
            execvp("diff", arrdiff);
            break;
        case COMANDO_T:
            char* arr[] = {"./student-records", "-i", "students-db.txt", "-p", NULL};
            execv("./student-records", arr);
            break;
        case COMANDO_A:
            int pid_1 = fork();
            ...
            if (pid_1 == 0) {
                char* arr[] = {"./student-records", "-t", NULL};
                execv("./student-records", arr);
            } else {
            ...
            if (pid_2 == 0) {
                char* arr[] = {"./student-records", "-i", "students-db.txt", "-o", options.output_file, "-b", NULL};
                execv("./student-records", arr);
            } else {
                printf("Ha habido un error\n");
            }
            ...
            printf("Procesamiento de comandos -b y -t realizado\n");

            pid = fork();

            if (pid == 0) {
                // Proceso hijo
                char* diff_final[] = {"diff", "cmp1.bin", "cmp2.bin", NULL};
                execvp("diff", diff_final);
                perror("execvp"); // Por si falla
                exit(EXIT_FAILURE);
            } else {
                // Proceso padre
                int status;
            ...
            char* diff_final[] = {"diff", options.output_file, "cmp1.bin", NULL};
            execvp("diff", diff_final);
            break;
        default:
            break;
